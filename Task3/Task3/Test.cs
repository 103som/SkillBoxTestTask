...
namespace Task3
{
    ...
    internal class Test
    {
        ...
        public void TestManagerFunctionality(string clientId)
        {
            Employer employer = new Manager("Manager", "0");

            Console.WriteLine(employer.GetSurname(clientId) + ' ' + employer.GetName(clientId) + ' ' +
                              employer.GetPatronymic(clientId) + ' ' + (employer as Manager).GetPassportData(clientId) + ' ' +
                              employer.GetPhoneNumber(clientId));


            (employer as Manager).ChangePatronymic("12345678", clientId);
            (employer as Manager).ChangeName("AAA", clientId);
            (employer as Manager).ChangeSurname("BBB", clientId);
